            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                {beautyData.map((beauty, index) => (
                    <div key={index} className="flex flex-col justify-between items-center">
                        <img src={beauty.icon} alt={`Logo ${index + 1}`} className="w-64 h-64 object-cover rounded-lg "
                        ...
                        <button
                            className="flex items-center justify-center w-24 h-12 bg-gray-200 hover:bg-violet-400 hover:text-white border
                             border-black rounded-lg transition duration-300 relative group" onClick={() => addToCart(beauty)}
                        >
                            <span className="absolute group-hover:hidden text-gray-700">₹{beauty.price}</span>
